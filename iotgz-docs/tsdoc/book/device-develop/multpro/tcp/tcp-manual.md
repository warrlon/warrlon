# TCP Access Instructions

** Step1 Create Products, Choose TCP Transfer**

First, you need to create a ** access product for TCP ** transmission on the platform and view the product creation (/ book/easy-manual/product&device/product-create.md)
...
Click on ** Add device **, enter ** device name ** and ** authentication information ** (i.e. device number). For details of the resource model of the specific platform, please refer to Chapter 1[resource model](/book/introduction/resource-model.md) and record the ** device number **.

** Step3 Upload Lua script**

Upload the parsing script to the product through the page, as shown in the following figure:
...
>- "hello" is sent every 5 seconds after the device is connected
>- After the user uploads the data, the platform replies to "received"
>- Converting user-uploaded data into HEX strings and storing them as data points into * ds_test * data streams
...
Among them:
Device_data_analysis (dev) is the analytic function of the terminal upstream data, and each packet of data will be processed.
Device_timer_init (dev) is a timer initialization function for specifying periodic data downloading operations.

** Step4 Connection Establishment**

When the device is powered on, it establishes a TCP connection with OneNET access service (domain name: ** dtu. heclouds. com **) and sends the login message in the following format:
...
- PID: Product ID, product unique digital identification generated by OneNET when creating a product
- AUTH_INFO: Device authentication information, the unique string identifier specified by the user when creating the device at the time of device registration
- PARSER_NAME: User-defined name of parsing script, unique string identifier specified when user uploads script

If the connection is successful, you can see the device online tag:
