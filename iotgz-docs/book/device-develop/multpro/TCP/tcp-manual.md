...
首先您需要在平台创建一个**接入为TCP透传**的产品，查看[产品创建](/book/easy-manual/product&device/product-create.md)
如果您还没有OneNET账号，可以[用户注册](/book/easy-manual/login.md)

**Step2 创建设备，记录设备ID等信息**

通过页面点击**添加设备**，输入**设备名称**和**鉴权信息**（即设备编号），具体平台的资源模型可详情请查看第一章 [资源模型](/book/introduce/resource-model.md)，并记录下该**设备编号**.
...
脚本示例：（[点击下载](/images/TCP/sample--str.lua)）
...
function device_timer_init(dev)
        ...
        -- 例如： --
--        dev:timeout(3)
--        dev:add(10,"ds_test","hello")    --每10秒下发一包数据，内容为hello
		dev:set_keep_alive(60)
end
...
function device_data_analyze(dev)
        ...
        add_val(t,"ds_id",a,dev:bytes(1,s)) --添加到datastream数据流
